...
// ─── Mini horizontal bar ─────────────────────────────────────────────────────
function MiniBar({ label, value, max, color, pct }) {
    const width = max > 0 ? (value / max) * 100 : (pct ?? 0);
    return (
...
export default function AdminReports() {
    ...
    const filtered = useMemo(() => {
        return tickets.filter((t) => {
            ...
            if (selTech !== "All" &&
                t.assignedTechnician !== selTech &&
                t.resolvedBy !== selTech) return false;
            return true;
        });
    ...
    const exportExcel = () => {
        ...
        XLSX.writeFile(wb, `DA-MIMAROPA-IT-Tickets-${datePart}.xlsx`);
    };
    ...
    return (
        <div className="p-6 space-y-6 max-w-7xl mx-auto">
            ...
            <div className="bg-white rounded-xl border border-gray-100 overflow-hidden">
                ...
                {showFilters && (
                    <div className="border-t border-gray-100 px-5 py-4 space-y-5">
                        ...
                        {/* Row 2: Status + Urgency toggles + Technician */}
                        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                                ...
                                <div className="flex flex-wrap gap-1.5">
                                    {STATUS_OPTIONS.map(s => (
                                        <button
                                            key={s}
                                            type="button"
                                            onClick={() => toggleMulti(setSelStatus, s)}
                                            className={`px-3 py-1.5 rounded-lg text-xs font-medium border transition-all ${selStatus.includes(s)
                                ...
                                <div className="flex flex-wrap gap-1.5">
                                    {URGENCY_OPTIONS.map(u => (
                                        <button
                                            key={u}
                                            type="button"
                                            onClick={() => toggleMulti(setSelUrgency, u)}
                                            className={`px-3 py-1.5 rounded-lg text-xs font-medium border transition-all ${selUrgency.includes(u)
                                                    ? "bg-emerald-700 text-white border-emerald-700"
                                                    : "bg-white text-gray-600 border-gray-200 hover:border-gray-300"
                                                }`}
                                        >
                            ...
                            <div>
                                ...
                                <select
                                    value={selTech}
                                    onChange={e => setSelTech(e.target.value)}
                                    className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm text-gray-700 outline-none focus:ring-2 focus:ring-emerald-100 focus:border-emerald-400 bg-white"
                                >
                                    {techOptions.map(o => (
                                        <option key={o} value={o}>{o === "All" ? "All personnel" : o}</option>
                                    ))}
                                </select>
                            </div>
                        </div>
                        ...
                        {activeFilterCount > 0 && (
                            <div className="flex items-center gap-2 flex-wrap pt-1">
                                ...
                                <button
                                    onClick={clearAll}
                                    className="text-xs text-gray-400 hover:text-red-500 font-medium transition ml-1"
                                >
                                    Clear all
                                </button>
                            </div>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-5 gap-3">
                ...
                <StatCard icon={Clock} label="Open" value={stats.open} iconBg="#eff6ff" iconColor="#1d4ed8"
            ...
            <div className="bg-white rounded-xl border border-gray-100 overflow-hidden">
                ...
                {sorted.length === 0 ? (
                    <div className="text-center py-16">
                        ...
                        <button onClick={clearAll} className="mt-2 text-xs text-emerald-700 hover:underline">
                            Clear filters
                        </button>
                    </div>
                ) : (
                                ...
                                {sorted.map((t) => (
                                    <tr key={t._docId} className="hover:bg-gray-50/60 transition-colors">
                                        ...
                                        <td className="px-4 py-3 whitespace-nowrap">
                                            <span className={`inline-block px-2 py-0.5 rounded-md text-[11px] font-medium ${URGENCY_BADGE[t.urgency] || "bg-gray-100 text-gray-500"}`}>
                                                {t.urgency}
                                            </span>
                                        </td>
                                        <td className="px-4 py-3 whitespace-nowrap">
                                            <span className={`inline-block px-2 py-0.5 rounded-md text-[11px] font-medium ${STATUS_BADGE[t.status] || "bg-gray-100 text-gray-500"}`}>
                                                {t.status}
                                            </span>
                                        </td>
                                        ...
                                        <td className="px-4 py-3 whitespace-nowrap">
                                            {t.resolvedBy
                                                ? <span className="flex items-center gap-1 text-emerald-700">
                                                    <UserCheck className="w-3 h-3" />{t.resolvedBy}
                                                </span>
                ...
                {sorted.length > 0 && (
                    <div className="px-5 py-3 border-t border-gray-100 flex items-center justify-between">
                        ...
                        <div className="flex gap-2">
                            ...
                            <button
                                onClick={exportExcel}
                                className="flex items-center gap-1.5 text-xs font-medium text-emerald-700 hover:text-emerald-900 border border-emerald-200 bg-emerald-50 hover:bg-emerald-100 px-3 py-1.5 rounded-lg transition"
                            >
